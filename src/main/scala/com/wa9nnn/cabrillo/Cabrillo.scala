...
import com.wa9nnn.cabrillo.requirements.{CabrilloError, Rules, RulesEngine}
import com.wa9nnn.cabrillo.requirements.{CabrilloError, Rules, RulesEngine}
...
object Cabrillo {
  ...
  def apply(bytes: Array[Byte]): ResultWithData = {
    ...
    cabrillo.ResultWithData(Result(callSign = cabrilloData("CALLSIGN").headOption.map(_.body), duration = duration, linesInFile = cabrilloData.inLineOrder.size, qsoCount = qsoCount, errors, unknowns),
      cabrilloData)
  }
}
